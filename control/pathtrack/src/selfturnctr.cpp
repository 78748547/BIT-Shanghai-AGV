...
void TSelfTurnCtr::Run() //  主运行函数
{
    ...
    else if (self_turn_ctr == 1) //  转动至角度误差小于阈值
    {
        ...
        float max_err = 6;      // 最小误差角
        float turn_speed = 0.2;
        float true_turn_speed;
        if(move_dir=="back")  turn_speed*=-1;
        if (speedlimit < 0.01) car_ctr.speed = 0;
        // ROS_INFO("track_angle_err=%2f",track_angle_err);
        if (track_angle_err> max_err)  
        {
            true_turn_speed = turn_speed*(fabs(track_angle_err)/90);
            if (fabs(true_turn_speed)<0.1) true_turn_speed = fabs(true_turn_speed)/true_turn_speed*0.1; 
            car_ctr.speed = -true_turn_speed;
        }
        else if (track_angle_err< -max_err)  
        {
            true_turn_speed = turn_speed*(fabs(track_angle_err)/90);
            if (fabs(true_turn_speed)<0.1) true_turn_speed = fabs(true_turn_speed)/true_turn_speed*0.1; 
            car_ctr.speed = true_turn_speed;
